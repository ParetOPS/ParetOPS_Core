...
function expandKPI(name) {
    ...
    const machineFilterSection = document.getElementById("machineFilterContainer")?.closest('.mt-3');
    ...
    if (!machineDataByKpi[name]) {
        ...
        machineList.forEach(machine => {
            machineDataByKpi[name][machine] = kpiMachineOverlays[name]?.[machine] || emptyWeek.slice();
        });
    }
  ...
  fetch('/api/reports')
    ...
    .then(files => {
      ...
      files.forEach(filename => {
        const display = filename.replace(".pdf", "");
...
async function saveExecutiveSummaryPdf() {
  ...
  const companyName = CONFIG.company?.name || 'ParetOPS';
  const reportPrefix = (companyName || 'ParetOPS').replace(/[^A-Za-z0-9]+/g, '_');
  const filename = `${reportPrefix}_Summary_${formattedDate}.pdf`;
  ...
  // Encode logo from the public/images folder
  ...
  <body>
    ...
    <div class="subtitle">${companyName} – ${monthLabel}</div>
    <div class="notice">
      This report was auto-generated by the <strong>ParetOPS (©)</strong> algorithm for <strong>${companyName}</strong>. It should be interpreted alongside the full KPI dashboard available on 
      <a href="http://localhost:3000/kpi_dashboard.html" style="text-decoration: none; color: #007bff;"><strong>ParetOPS portal</strong></a>.
    </div>
    ...
    <div class="confidential">
      This report contains confidential information. All data and analyses included are the exclusive property of ${companyName} and may not be shared or reproduced without prior authorization.
    </div>
  </body>
  </html>`;
...
// Unified machine list helper (config first, then any keys from overlays)
function getMachineList() {
  if (MACHINES.length) return MACHINES;
  const all = new Set();
  Object.values(kpiMachineOverlays || {}).forEach(map => {
    Object.keys(map || {}).forEach(m => all.add(m));
  });
  return Array.from(all);
}

// Build machine filter checkboxes based on the unified machine list
function renderMachineFilters() {
  ...
  machines.forEach(machine => {
    const safeId = `machine-${machine.replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase() || 'm'}`;
    const wrapper = document.createElement("div");
    wrapper.className = "form-check form-check-inline";

    const input = document.createElement("input");
    input.type = "checkbox";
    input.className = "form-check-input machine-filter";
    input.id = safeId;
    input.value = machine;
    input.onchange = handleMachineShiftExclusivity;
...
function startProductionKpiApp() {
  ...
  loadRealKpiData()
    ...
    .then(() => {
      ...
      // Build machine filters now that config/overlays are loaded
