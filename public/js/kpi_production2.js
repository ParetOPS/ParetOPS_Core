...
const kpiMachineSelections = {};  // stores selected machines per KPI
const thresholds = {}; // Store user-defined threshold per KPI
const goals = {}; // Store user-defined goal (maximize/minimize) per KPI
...
const machineData = {
  PC1: [], PC2: [], PC4: [], PC5: [], PC6: [], PC7: [], PC8: [], PC9: []
};
  ...
  kpiList.forEach(kpi => {
    ...
    if (kpi.name === 'Yield (%)') {
      ...
      labels.forEach((l, idx) => {
        const asDate = new Date(l);
        const austinDate = new Date(asDate.toLocaleString("en-US", { timeZone: "America/Chicago" }));

        console.log("📅 Frontend label parsing:");
        console.log("Raw label string:", l);
        console.log("Interpreted UTC date:", asDate.toString());
        console.log("Converted to Austin:", austinDate.toString());
      });
      labels.forEach((l, idx) => {
        console.log("📅 l = ", l);

        const date = new Date(`${l}T00:00:00Z`);
    ...
    selectedMachines.forEach(machine => {
      ...
      rawLabels.forEach((l, idx) => {
        ...
        const weekday = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][local.getDay()];
        const formatted = `${weekday} ${local.getMonth() + 1}/${local.getDate()}`;
console.log("✅ Formatted label:", formatted);        machineLabelToValue[formatted] = rawMachineValues[idx];
      });
...
function updateHighlightGrid() {
  ...
  highlights.forEach(kpiName => {
    ...
    const labels = kpiName === 'Yield (%)' ? kpiData.extendedLabels : kpiData.labels;
    const values = kpiName === 'Yield (%)' ? kpiData.extendedValues : kpiData.values;
    const rawLabels = labels;

    // Determine visible range (last 7 days)
    const today = new Date();
    const past7Labels = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date(today);
    ...
    // Match labels to values to get visible data
    const labelMap = {};
    labels.forEach((l, idx) => {
      const date = new Date(`${l}T00:00:00Z`);
      const local = new Date(date.toLocaleString("en-US", { timeZone: "America/Chicago" }));
      const weekday = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][local.getDay()];
      const formatted = `${weekday} ${local.getMonth() + 1}/${local.getDate()}`;
console.log("✅ Formatted label:", formatted);      labelMap[formatted] = values[idx];
    });

    const visibleLabels = past7Labels;
    const visibleData = past7Labels.map(l => labelMap[l] ?? null);

    // Compute delta from last two non-null visible values
    let lastValid = null, prevValid = null;
    for (let i = visibleData.length - 1; i >= 0; i--) {
      if (visibleData[i] != null) {
        if (lastValid === null) {
...
function mapShiftToLabels(rawLabels, rawShiftValues, targetLabels, formatFn) {
  ...
  rawLabels.forEach((label, i) => {
    const formatted = formatFn(new Date(label));
...
function setTimeframe(days) {
  ...
  // Build list of dates between pastDate and today
  const fullDates = [];
  const monthMarkers = [];
  ...
  const displayLabels = fullDates.map(label => {
    const date = new Date(label);
  ...
  const labelToValue = {};
  labels.forEach((l, i) => {
    const formatted = new Date(l).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" });
    labelToValue[formatted] = values[i];
  });
  ...
  if ((kpiShiftSelections[currentKpi] || []).includes("shift1") && currentShiftData["shift1"])
   shiftOverlay["shift1"] = mapShiftToLabels(rawLabels, currentShiftData["shift1"], fullDates, d => d.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift2") && currentShiftData["shift2"])
    shiftOverlay["shift2"] = mapShiftToLabels(rawLabels, currentShiftData["shift2"], fullDates, d => d.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift3") && currentShiftData["shift3"])
    shiftOverlay["shift3"] = mapShiftToLabels(rawLabels, currentShiftData["shift3"], fullDates, d => d.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));
...
function setCurrentWeek() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    past7Days.push(formatted); // Used for internal matching
    displayLabels.push(d.toLocaleDateString("en-US", { month: "short", day: "numeric" })); // Displayed on chart
  }
  ...
  labels.forEach((label, idx) => {
    const formatted = new Date(label).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" });
    labelToValue[formatted] = values[idx];
  });
  ...
  if ((kpiShiftSelections[currentKpi] || []).includes("shift1") && currentShiftData["shift1"]) 
    shiftOverlay["shift1"] = mapShiftToLabels(rawLabels, currentShiftData["shift1"], past7Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift2") && currentShiftData["shift2"]) 
    shiftOverlay["shift2"] = mapShiftToLabels(rawLabels, currentShiftData["shift2"], past7Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift3") && currentShiftData["shift3"]) 
    shiftOverlay["shift3"] = mapShiftToLabels(rawLabels, currentShiftData["shift3"], past7Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));
...
function setCurrentMonth() {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    d.setDate(today.getDate() - i);
    const formatted = d.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" });
    past30Days.push(formatted); // Internal use
    displayLabels.push(d.toLocaleDateString("en-US", { month: "short", day: "numeric" })); // Displayed on chart
  }
  ...
  if ((kpiShiftSelections[currentKpi] || []).includes("shift1") && currentShiftData["shift1"])
    shiftOverlay["shift1"] = mapShiftToLabels(rawLabels, currentShiftData["shift1"], past30Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift2") && currentShiftData["shift2"])
    shiftOverlay["shift2"] = mapShiftToLabels(rawLabels, currentShiftData["shift2"], past30Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));

  if ((kpiShiftSelections[currentKpi] || []).includes("shift3") && currentShiftData["shift3"])
    shiftOverlay["shift3"] = mapShiftToLabels(rawLabels, currentShiftData["shift3"], past30Days, d => new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" }));
...
function setYTD() {
  ...
  for (let i = 1; i <= totalWeeks; i++) {
    ...
    if (weeklyCounts[week] && weeklyCounts[week] > 0) {
      dataYTD.push((weeklySums[week] / weeklyCounts[week]).toFixed(1)); // ✅ average, not sum
    } else {
      dataYTD.push(null);
    }
  }
...
const weekday = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][local.getDay()];
const formatted = `${weekday} ${local.getMonth() + 1}/${local.getDate()}`;
console.log("✅ Formatted label:", formatted);    labelMap[formatted] = rawValues[idx];
  });
...
const weekday = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"][local.getDay()];
const formatted = `${weekday} ${local.getMonth() + 1}/${local.getDate()}`;
console.log("✅ Formatted label:", formatted);      machineLabelToValue[formatted] = rawMachineValues[i];
    });
...
function saveExecutiveSummaryPdf() {
  ...
  const end = new Date(today);
  end.setDate(today.getDate() - 1); // D-1
  const start = new Date(today);
  start.setDate(today.getDate() - 30); // D-30
  ...
  <body>
    ...
    <div class="header">
      <img src="file:///C:/inetpub/wwwroot/ParetOPS_v3.0.0/public/images/paretops.svg" alt="ParetOPS logo" style="height: 80px;">
      <img src="file:///C:/inetpub/wwwroot/ParetOPS_v3.0.0/public/images/cfan.png" alt="CFAN logo" style="height: 80px;">
    </div>
    ...
    <div class="notice">
      This report was auto-generated by the <strong>ParetOPS (©)</strong> algorithm. It should be interpreted alongside the full KPI dashboard available on <a href="http://localhost:3000/kpi_dashboard.html" style="text-decoration: none; color: #007bff;"><strong>ParetOPS: portail CFAN</strong></a>.
    </div>
...
function openMonthlyReportOverlay() {
  ...
  overlay.innerHTML = `
    ...
    ">
      ...
      <button class="btn btn-primary w-100 mb-3" style="font-weight: 600;" onclick="saveExecutiveSummaryPdf()">
        <i class="fas fa-file-alt me-2"></i>Generate New Monthly Report
      </button>
